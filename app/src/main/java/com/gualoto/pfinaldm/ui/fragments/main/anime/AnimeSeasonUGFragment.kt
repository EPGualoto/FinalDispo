...
import com.gualoto.pfinaldm.databinding.FragmentAnimeSeasonUGBinding
import com.gualoto.pfinaldm.ui.core.SeasonUAnimeUI
import java.text.NumberFormat
import java.util.Locale

class AnimeSeasonUGFragment : Fragment() {
    private lateinit var binding: FragmentAnimeSeasonUGBinding

    override fun onCreateView(
        inflater: LayoutInflater, container: ViewGroup?,
        savedInstanceState: Bundle?
    ): View {
        binding = FragmentAnimeSeasonUGBinding.inflate(inflater, container, false)
        return binding.root
    }
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        val data: SeasonUAnimeUI? = jsonString?.let {
            Gson().fromJson(it, SeasonUAnimeUI::class.java)
        }
        ...
        data?.let { anime ->
            ...
            val allGenres =
                anime.genres?.map { genre -> genre.name }
                    ?.plus(anime.themes?.map { theme -> theme.name })
